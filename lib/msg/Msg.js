...
class Msg {
    static byKey = {};
    static from(sourceOn, initValue, sourceHandler) {
        const target = new Msg(initValue);
        target.sOn = sourceOn;
        target.sHandler = (sourceHandler && sourceHandler(target)) || ((value) => target.set(value));
        return target;
    }
    ...
    map(cb, sourceHandler) {
        const source = this;
        return Msg.from(h => source.on(h), cb(source.v), target => sourceHandler ? sourceHandler(target) : (value) => target.set(cb(value)));
    }
    debounce(ms) {
