...
export const reqFetch = async <T = any>(ctx: RContext<T>): Promise<void> => {
    try {
        ...
        switch (ctx.responseType) {
            case 'blob': ctx.data = await response.blob() as T; break;
            case 'json': ctx.data = await response.json() as T; break;
            case 'text': ctx.data = await response.text() as T; break;
            case 'arraybuffer': ctx.data = await response.arrayBuffer() as T; break;
        }
    }
    catch (error) {
        ctx.error = error;
